...
lisp = frozenset(['lisp'])
ocaml = frozenset(['ml'])
reasonml = frozenset(['re'])
py = frozenset(['py'])
R = frozenset(['R', 'Rmd'])
sql = frozenset(['hql', 'sql'])
clang = frozenset(['c'])

file_types = {adoc: 'asciidoc', clang: 'c', fs: 'fsharp', hs: 'haskell',
              julia: 'julia', lisp: 'lisp', ocaml: 'ocaml',
              reasonml: 'reasonml', py: 'python', R: 'R', sql: 'sql'}

usage = """Usage: vi -h | <file> [as <extension>]
Examples: vi my-script.py
          vi my-script as py
Supported extensions:\n  * """ + '\n  * '.join(
    [', '.join(lang) for lang in file_types])
